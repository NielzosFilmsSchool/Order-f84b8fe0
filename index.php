...
try {
     ...
     $table = "<table><tr><th><a href='?sort=title' >Titel</a></th><th><a href='index.php?sort=rating' >Rating</a></th><th>Details</th></tr>";
     ...
     if(isset($_GET["sortFilms"])){
          ...
          if($_GET["sortFilms"] == "duur")
               $stmt = $pdo->query('SELECT titel, duur FROM films ORDER BY duur DESC');
     }else
          $stmt = $pdo->query('SELECT titel, duur FROM films');
     $table = "<table><tr><th><a href='?sortFilms=title' >Titel</a></th><th><a href='index.php?sortFilms=duur' >Duur</a></th><th>Details</th></tr>";
     while ($row = $stmt->fetch())
     {
